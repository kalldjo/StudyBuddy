...
const getMe = async (req, res) => {
  try {
    ...
    const query = `
      ...
      WITH u, j, f, a, collect(s.name) AS skills
      RETURN u {
        .*, 
        jurusan: j.name, 
        fakultas: f.name, 
...
const handleSocialBypass = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const defaultAvatar = provider === "Google" ? "https://lh3.googleusercontent.com/a/default-user" : "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png";
      user = await authModel.createUser(id, email, passwordHash, name || `${provider} Scholar`, `Classmate authenticated securely via ${provider} integrations.`, defaultAvatar);
    }
