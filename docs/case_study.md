...
Study Buddy maps directly to LinkedIn's approach but is tailored for the university academic ecosystem. By structuring our data in **Neo4j**, we represent students (`User`), their academic demographics (`Fakultas`, `Jurusan`, `Angkatan`), and capabilities (`Skill`, `Interest`, `MataKuliah`) as a rich social graph. This allows our recommendation engine to use Cypher queries to instantly find and match students for project collaboration and study groups based on real-time graph traversals.
